...
import { Upload, X, Send, Image, FileText, Video, Music, AlertCircle } from 'lucide-react';
...
const MediaSender: React.FC<MediaSenderProps> = ({ selectedChat, onClose }) => {
  ...
  const validateFile = useCallback((file: File, mediaType: string) => {
    ...
    // For document types with extensions
    ...
    // For other media types that use MIME types
    else if (!acceptTypes.some(type => file.type.match(type))) {
      throw new Error(`Format de fichier non supporté. Formats acceptés: ${mediaTypeConfig.accept}`);
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      // Upload the media
      const mediaResult = await uploadMedia(file, mediaType);
      setUploadProgress(50);
      
      // Send the media message
      await sendMediaMessage(selectedChat.id, mediaType, mediaResult.mediaId, caption);
